...
class Planner extends Agent {
  ...
  constructor({ task, loop }) {
    const persona = `Using the given Context, create a precise plan for the given Task. This plan will be executed by others and your responsibility is to produce a plan that reflects the job. 
...
`;
    ...
    effect(() => {
      // Parse the response from the super.
      const response = super.response.value;
      ...
      try {
        ...
        effect(() => {
          ...
          if (loopOutput == Command.STOP) {
            ...
            this.#finalResponse.value = loopResult;
            return;
          }
          ...
          if (todo.length == 0 && processing == false) {
            // end the task;
            loopResult.at(-1).immediateResult = loopOutput;
          ...
          if (todo.length > 0 && processing == false) {
            ...
            // push the task on to the loopResult with an empty immediateResult
            loopResult.push({ task: item.task });
            // Start the task.
